...
get_da_result <- function(input_tbl,DA_method,fig_type,sig_tag) {
  ...
  plt <-
    ggplot(clean_tbl,mapping = aes(x = haplotype,y = trait,fill = haplotype)) +
                         ...
                         if(ht_obj$order_method == "hclust") {
                           ht_obj$ht =
                             Heatmap(
                               ...
                               cluster_columns = F,cluster_rows = T,use_raster = T,
                           ...
                           ht_obj$ht =
                             Heatmap(
                               ...
                               col = circlize::colorRamp2(breaks = c(0,1,2,3),colors = ht_obj$colorkey),
                               show_row_names = F,show_column_names = F,
                               cluster_columns = F,cluster_rows = ht_obj$tree_clado,use_raster = T,
                               heatmap_legend_param = list(
                                 color_bar = "discrete",
                                 labels = c("Missing","Heterozygous","Minor","Major"),
                                 title = "Genotype"
