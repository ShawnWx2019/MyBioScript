...
command=matrix(c(
  ...
  'cyc_db', 'c', 1, 'character', 'plantcyc database, if If the species you do has a corresponding cyc database, use the corresponding database, such as aracyc, else use plantcyc database',
...
if (is.null(args$cyc_db)){
  message(msg_no("Need xxcyc database file! please read help carefully!"))
  message(msg_yes(paste(getopt(command, usage = T), "\n")))
  q(status = 1)
}
...
##test
if (TEST == "TRUE") {
  peak_path = "/Users/shawnwang/15.PostDoc/02.Project/cyk/04.cyk/raw_peak.csv"
  group_path = "/Users/shawnwang/15.PostDoc/02.Project/cyk/04.cyk/group_info.csv"
  annotation_path = "/Users/shawnwang/15.PostDoc/02.Project/cyk/04.cyk/meta_anno.txt"
  VIP_cut = 1;
  left = "myb_tri";
  right = "col";
  pval_cut = 0.05;
  qval_cut = 1;
  log2fc_cut = 0;
  load("~/02.MyScript/PlantCyc_MS1_DB/02.Rdata/ath_cyc.db")
  load("~/02.MyScript/PCK_Db_github/02.Rdata/ath.db")
} else {
...
ht_DAM_log2area = Heatmap(
  matrix = log2(heat_mat),
  name = "log2(peak area)",
  show_row_names = F,cluster_columns = F,
  col = colorRamp2(c(min(log2(heat_mat)), max(log2(heat_mat))), c("yellow", "#FF1493")),
  border_gp = gpar(color = "black",size = 1)
)
...
# start plantcyc enrichment -----------------------------------------------

DAM_clean %>% 
  select(CompoundID,mf,mw,Name) %>% 
  distinct() %>% 
  inner_join(.,cyc_db,by = c("mf" = "Chemical_formula")) %>% 
  mutate(judge = case_when(
    abs(mw - Molecular_weight) < 10 ~ "T",
    TRUE ~ "F"
  )) %>% 
  filter(judge == "T") %>% 
  distinct()-> DAM_cyc_match
DAM_cyc_match %>% 
  aggregate(.,by = list(.$CompoundID), c) %>% transmute(
    CompoundID = .$Group.1,
    CD_Name = map_chr(.$Name,.f = function(.x) {unique(.x)}),
    mf = map_chr(.$mf,.f = function(.x) {unique(.x)}),
    mw = map_chr(.$mw,.f = function(.x) {unique(.x)}),
    cyc_id = map_chr(.$Compound_id,.f = function(.x) {paste0(unique(.x),collapse = " | ")}),
    cyc_name = map_chr(.$Compound_common_name,.f = function(.x) {paste0(unique(.x),collapse = " | ")}),
    Smiles = map_chr(.$Smiles,.f = function(.x) {paste0(unique(.x),collapse = " | ")}),
    Links = map_chr(.$Links,.f = function(.x) {paste0(unique(.x),collapse = " | ")}),
    EC. = map_chr(.$EC,.f = function(.x) {paste0(unique(.x),collapse = " | ")}),
    Reaction = map_chr(.$Reaction_equation,.f = function(.x) {paste0(unique(.x),collapse = " | ")}),
    cyc_TERM = map_chr(.$TERM,.f = function(.x) {paste0(unique(.x),collapse = " | ")}),
    Pathway = map_chr(.$Pathway,.f = function(.x) {paste0(unique(.x),collapse = " | ")})
  ) -> DAM_cyc_match_collapse
write.csv(DAM_cyc_match,file = paste0(store_path,"/10.DAM_plantcyc.csv"),row.names = F)
write.csv(DAM_cyc_match_collapse,file = paste0(store_path,"/10.DAM_plantcyc_collapse.csv"),row.names = F)

cyc_list = DAM_cyc_match$Compound_id %>% unique()

message(msg_run("Running plantcyc enrichment ..."))

cko <- enricher(
  gene = cyc_list,pvalueCutoff = 0.05,qvalueCutoff = 1,TERM2GENE = cyc_t2g,TERM2NAME = cyc_t2n
)
cko_tbl <- as.data.frame(cko)
if(nrow(cko_tbl) == 0){
  message(msg_no("There are no significant enriched Plantcyc pathway according to detected DAMs!"))
} else {
  write.csv(cko_tbl,paste0(store_path,"/10.DAM_plantcyc_enrichment.csv"),row.names = F)
  cko_tbl_long = cko_tbl %>% 
    select(Description,ID,geneID) %>% 
    cSplit(.,"geneID","/","long") %>% 
    inner_join(.,DAM_cyc_match,by = c("geneID" = "Compound_id")) %>% 
    select(ID,Description,CompoundID,Name,Compound_common_name,Compound_synonyms,geneID,mf,mw) %>% 
    mutate(
      Compound_common_name = gsub("<.*>","",Compound_common_name),
      Compound_synonyms = gsub("<.*>","",Compound_synonyms)
    ) %>% 
    set_names(c("PlantCyc_PathwayID","PlantCyc_Pathway","CompoundID","CD_annotation","PlantCyc_Compound_annotation","PlantCyc_Compound_synonyms","PlantcCyc_CompoundID","Molecular_formula","Molecular_weight")) %>% 
    distinct() %>% 
    left_join(.,DAM_tbl_all,by = "CompoundID") %>% 
    left_join(.,as.data.frame(heat_mat) %>% rownames_to_column("CompoundID"),by = "CompoundID") %>% 
    distinct()
  write.csv(cko_tbl_long,paste0(store_path,"/10.DAM_plantcyc_enrichment.expand.csv"),row.names = F)
  cyc_barplt = barplot(cko,showCategory = 20) + ggtitle(paste0(left," vs ",right))
  ggsave(plot = cyc_barplt,filename = paste0(store_path,"/10.DAM_plantcyc_enrichment_bar.pdf"),height = 16,width = 12)
  ggsave(plot = cyc_barplt,filename = paste0(store_path,"/10.DAM_plantcyc_enrichment_bar.png"),height = 16,width = 12)
  cyc_bubbplt = dotplot(cko,showCategory = 20)+ ggtitle(paste0(left," vs ",right))
  ggsave(plot = cyc_bubbplt,filename = paste0(store_path,"/10.DAM_plantcyc_enrichment_bubble.pdf"),height = 16,width = 12)
  ggsave(plot = cyc_bubbplt,filename = paste0(store_path,"/10.DAM_plantcyc_enrichment_bubble.png"),height = 16,width = 12)
  
  ht_cyc_path_1  = cko_tbl_long %>% 
    select(CompoundID,starts_with(paste0(left,"_rep")),starts_with(paste0(right,'_rep'))) %>% 
    distinct() %>% 
    arrange(CompoundID) %>% 
    column_to_rownames("CompoundID") %>% 
    as.matrix() %>% t() %>% scale() %>% t() %>% 
  ...
  pdf(file = paste0(store_path,"/11.PlantCyc_Enrichment_heatmap.pdf"),width = 17,height = 15)
  draw(ht_cyc_enrich)
  invisible(dev.off())
  png(file = paste0(store_path,"/11.PlantCyc_Enrichment_heatmap.png"),width = 2700,height = 2500,res = 300)
  draw(ht_cyc_enrich)
  invisible(dev.off())
}


# plantcyc pathway analysis -----------------------------------------------

message(msg_run("Running plantcyc pathway analysis..."))

cko <- enricher(
  gene = cyc_list,pvalueCutoff = 1,qvalueCutoff = 1,TERM2GENE = cyc_t2g,TERM2NAME = cyc_t2n
)
cko_tbl <- as.data.frame(cko)
if(nrow(cko_tbl) == 0){
  message(msg_no("No DAMs can matched with plantcyc database"))
} else {
  write.csv(cko_tbl,paste0(store_path,"/12.DAM_plantcyc_Pathway.csv"),row.names = F)
  cko_tbl_long = cko_tbl %>% 
    select(Description,ID,geneID) %>% 
    cSplit(.,"geneID","/","long") %>% 
    inner_join(.,DAM_cyc_match,by = c("geneID" = "Compound_id")) %>% 
    ...
    left_join(.,as.data.frame(heat_mat) %>% rownames_to_column("CompoundID"),by = "CompoundID") %>% 
    distinct()
  write.csv(cko_tbl_long,paste0(store_path,"/12.DAM_plantcyc_Pathway.expand.csv"),row.names = F)
  cyc_barplt = barplot(cko,showCategory = 20)+ggtitle(paste0(left," vs ",right))
  ggsave(cyc_barplt,filename = paste0(store_path,"/12.DAM_plantcyc_pathway_bar.pdf"),height = 16,width = 12)
  ggsave(cyc_barplt,filename = paste0(store_path,"/12.DAM_plantcyc_pathway_bar.png"),height = 16,width = 12)
  cyc_bubbplt = dotplot(cko,showCategory = 20)+ggtitle(paste0(left," vs ",right))
  ggsave(cyc_bubbplt,filename = paste0(store_path,"/12.DAM_plantcyc_pathway_bubble.pdf"),height = 16,width = 12)
  ggsave(cyc_bubbplt,filename = paste0(store_path,"/12.DAM_plantcyc_pathway_bubble.png"),height = 16,width = 12)
  ht_cyc_path_1  = cko_tbl_long %>% 
    select(CompoundID,starts_with(paste0(left,"_rep")),starts_with(paste0(right,'_rep'))) %>% 
    distinct() %>% 
    arrange(CompoundID) %>% 
  ...
  ht_cyc_enrich = ht_cyc_path_1+ht_cyc_path_2+ht_cyc_path_3+ht_cyc_path_4+ht_cyc_path_5+ht_cyc_path_6
  pdf(file = paste0(store_path,"/12.PlantCyc_Pathway_heatmap.pdf"),width = 17,height = 15)
  draw(ht_cyc_enrich)
  invisible(dev.off())
  png(file = paste0(store_path,"/12.PlantCyc_Pathway_heatmap.png"),width = 2700,height = 2500,res = 300)
  draw(ht_cyc_enrich)
  invisible(dev.off())
}
