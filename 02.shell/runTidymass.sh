...
if [ -d "QC" ] || [ -d "Subject" ] || [ -d "POS" ] || [ -d "NEG" ]
then
    echo -e "\033[34mData orgniazation have already fininshed! start next step.\033[0m"
else
    if [ $type == "1" ] 
    then
        ls -ltr > injection_order.txt && mv injection_order.txt ../
        mkdir -p QC Subject
    ...
    then
        cd NEG && ls -ltr > injection_order_neg.txt && mv injection_order_neg.txt ../
        mkdir -p QC Subject
        mv QC*.raw QC && mv *.raw Subject 
        pos_dir_raw=$(realpath ./)
        cd ../POS && ls -ltr > injection_order_neg.txt && mv injection_order_neg.txt ../
        mkdir -p QC Subject
        mv QC*.raw QC && mv *.raw Subject && pos_dir_raw=$(realpath ./) && cd ../
        tree -d
